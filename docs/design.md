...
Status: living design contract. P1-P6 are implemented and verified, including both live discovery workflows; P7 also verifies the React console, review catalog and same-session handoff. The optional container profile has passed development replay and independent egress verification; final release evidence remains pending. PLAN.md records the latest passed gates. The schema descriptions below match the implemented v1; future behavior is identified by its stage.
