...
The final local verification at `11a8225` is preserved in [the containment summary](../evidence/verification/containment.json). All four checks pass. The subsequent `c7d617c` change only versions discovery prompt provenance and preserves older artifact compatibility; Linux CI `34651378928` passes the contained replay and egress check at that revision. Exact capture source is recorded separately in the evidence manifest.
