...
The final local verification at `bba5b6e` is preserved in [the containment summary](../evidence/verification/containment.json). Its runtime source matches the final capture at `67c4000`; the evidence manifest checks those file identities. Linux CI also passed at `67c4000`.
